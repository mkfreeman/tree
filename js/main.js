...
function regenerate() {
	...
	var text = status == 'built' ? 'Ungrow' : 'Grow'
	d3.select('.regenerate').text(text)
}
...
function create() {
	d3.select('svg')
		...
		.transition()
		.delay(function(d,i){return d.d*200})
		.duration(500)
		.attr('x2', x2)
		.attr('y2', y2)
...
function destroy() {
	d3.select('svg')
		.selectAll('line')
		.transition()
		.delay(function(d,i){return (10 - d.d) * 200})
		.duration(500)
		.attr('x2', x1)
		.attr('y2', y1)
		.remove()
